    ...
    for j in range(height): 
        ...
        if currPos["wallTop"] is False: 
            ...
            if nextPos["wallTop"] is False: 
                score = model.NewIntVar(0, 100, "light score of top of top of grid %s, %s]" % (i, j))
                model.Add(score == RADIUS_TWO_SCORE).OnlyEnforceIf(hasLamp[i - 2, j])
                model.Add(score == 0).OnlyEnforceIf(hasLamp[i - 2, j].Not())
                scores.append(score)
        ...
        if currPos["wallRight"] is False: 
            ...
            if nextPos["wallRight"] is False: 
                score = model.NewIntVar(0, 100, "light score of top of top of grid %s, %s]" % (i, j))
                model.Add(score == RADIUS_TWO_SCORE).OnlyEnforceIf(hasLamp[i, j + 2])
                model.Add(score == 0).OnlyEnforceIf(hasLamp[i, j + 2].Not())
                scores.append(score)
        ...
        if currPos["wallTop"] is False and currPos["wallRight"] is False: 
            score = model.NewIntVar(0, 100, "light score of left of grid[%s, %s]" % (i, j))
            model.Add(score == RADIUS_TWO_SCORE).OnlyEnforceIf(hasLamp[i - 1, j + 1])
            model.Add(score == 0).OnlyEnforceIf(hasLamp[i - 1, j + 1].Not())
            scores.append(score)
